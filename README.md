# DREAM.CAR.VAVD

Преміальний, тримовний (uk / ru / en) сайт автомобільних послуг DREAM.CAR.VAVD. Next.js App Router, TypeScript, Tailwind CSS v4.

## Запуск

```bash
npm install
npm run dev
```

Відкрий [http://localhost:3000](http://localhost:3000) — автоматично перенаправляє на `/uk`.

Якщо `next dev`/`next build` падають через нестачу оперативної пам'яті (Turbopack requires more RAM для збірки CSS), скористайся webpack-варіантом:

```bash
npm run dev:webpack
npm run build:webpack
```

## Скрипти

| Команда | Опис |
|---|---|
| `npm run dev` | Dev-сервер (Turbopack) |
| `npm run dev:webpack` | Dev-сервер (webpack, для машин з обмеженою RAM) |
| `npm run build` | Production-збірка (Turbopack) |
| `npm run build:webpack` | Production-збірка (webpack) |
| `npm run start` | Запуск production-збірки |
| `npm run lint` | ESLint |
| `npm run test` | Тести (`node --test` через `tsx`) — перевіряють узгодженість словників локалізації |

## Зображення

Поклади реальні файли за цими шляхами (заміняють плейсхолдери):

```
public/images/dream-car-logo.png    — логотип (шапка, hero, footer)
public/images/hero-cars-logo-final.jpg — фото автомобілів на головному екрані
public/images/reference-banner.png  — лише референс дизайну, на сайті не показується
public/images/gallery/01.jpg … 06.jpg — 6 фото загальної секції галереї
public/images/gallery/<album-id>/*.jpg — фотоальбоми проєктів (зараз: maserati-levante — 8 фото, volvo-xc60-d5 — 5 фото)
# gallery/07.jpg, gallery/08.jpg наразі на диску, кодом не використовуються
```

## Змінні середовища

Скопіюй `.env.example` у `.env.local` і заповни:

```
NEXT_PUBLIC_SITE_URL=              # для canonical/OG/sitemap
CONTACT_FORM_ENDPOINT=             # server-only секрет — URL доставлення форми зворотного зв'язку
NEXT_PUBLIC_INSTAGRAM_URL=
NEXT_PUBLIC_FACEBOOK_URL=
NEXT_PUBLIC_YOUTUBE_URL=
NEXT_PUBLIC_BUSINESS_ADDRESS=
NEXT_PUBLIC_GOOGLE_MAPS_URL=
```

`CONTACT_FORM_ENDPOINT` — **server-only секрет**, не префіксуй його `NEXT_PUBLIC_*`. Змінні з цим префіксом Next.js вбудовує у клієнтський JS-бандл і вони стають видимими будь-кому в браузері; для доставлення форми це неприпустимо. Не виводь це значення в лог, консоль чи UI, і не комітуй його — `.env*` вже в `.gitignore` (крім `.env.example`, де значення завжди порожнє).

WhatsApp і Telegram показуються завжди — їхні посилання захардкожені в `src/lib/social.ts` (`DEFAULT_CONTACT`; Telegram: `https://t.me/DREAM_CAR_VAVD`), без env-змінної, і панель може перекрити Telegram опублікованим контактним записом. Instagram/Facebook/YouTube показуються лише якщо відповідна `NEXT_PUBLIC_*`-змінна задана (або панель опублікувала своє значення). Форма зворотного зв'язку показується завжди: `ContactForm` завжди рендериться і надсилає POST на власний same-origin `/api/contact` (див. `src/app/api/contact/route.ts`), який валідує вхідні дані на сервері й лише тоді пересилає їх на `CONTACT_FORM_ENDPOINT`. Якщо змінна не задана, ендпоінт повертає `503 NOT_CONFIGURED`, і форма показує користувачу дружнє повідомлення замість помилки.

## Структура

```
src/
  app/
    [locale]/            # uk | ru | en — layout, головна, сторінки послуг, privacy, cookies
    sitemap.ts, robots.ts
  components/             # UI-компоненти (SiteHeader, HeroSection, ServicesGrid, ContactForm, ...)
  content/
    dictionaries/         # типізовані тексти для uk/ru/en
    services.ts           # метадані послуг (slug + іконка)
    types.ts              # тип Dictionary
  lib/
    i18n/                 # список локалей, завантаження словника
    social.ts             # посилання на соцмережі/телефон/email з env
```

## Мови

Три мовні версії `/uk`, `/ru`, `/en` з перемикачем у шапці, hreflang і canonical для кожної сторінки.
