...
**`71d2d28ed9cc982787dc647acb5b9661efdd9c35`**
(squash-коміт PR #26, `feat(panel): DREAM.CAR.VAVD content panel — cars +
gallery (text+media) + draft preview`, злито `2026-09-22T16:38:56Z`) — після
PR #30 (постійне Telegram-посилання, `2026-09-16`) і PR #31 (оновлення
безпеки залежностей, `2026-09-22`). Панель запущено в Production, вхід через
GitHub працює, повний hosted-цикл публікації через гілку `panel/content`
пройдено (`2026-09-26`). Докази — розділи 27–29.
Попередня контрольна точка: `8e262cd3166a32dffee49e99002fa11f59628ce3`
(squash-коміт PR #24, `fix: reliably return focus to the gallery card
after closing its modal`, злито `2026-09-05T19:46:50Z`) — після PR #17
(легша CSS-маска логотипу), PR #18 (CTA "консультація" в модалці послуги),
PR #19 (передача обраного авто у форму), PR #21 (той самий CTA в модалці
галереї), PR #20/#22/#23 (документація) — усі `2026-09-05`. Докази — розділ
21 (PR #17–#20), розділ 22 (PR #21), розділ 23 (PR #24 + приймання).
Хронологію продуктових, перформанс та observability-етапів після PR #5
(PR #8–#15) див. розділ 16; PR #17–#20 — розділ 21; зовнішній стан домену й
Google Search Console — розділ 17; favicon у Google — розділ 18;
впорядкований backlog A–E — розділ 19.
...
- Стара папка `/Users/apple/Desktop/MY PROEKT-2` є резервною історичною
  ...
  дозволу власника.
- **Базова контрольна точка `main` перед документаційною правкою — остання
  функціональна контрольна точка коду:**
  `71d2d28ed9cc982787dc647acb5b9661efdd9c35` (squash-коміт PR #26, злито
  `2026-09-22T16:38:56Z`). Стан на момент перевірки `2026-09-26`, до
  документаційного коміту: `origin/main` = цей коміт, активна гілка `main`,
  робоче дерево чисте. Required CI-check `Verify` на `main` — `success` після
  PR #30 (run `35098211263`), PR #31 (run `35734749277`) і PR #26 (run
  `35755509230`). Production: запис GitHub Deployments `6595803894`
  (sha `71d2d28`), останній статус — `success` о `2026-09-25T12:50:48Z`
  (повторний деплой після виправлення `KEYSTATIC_SECRET`, розділ 28).
  Відкритих PR і відкритих Dependabot-сповіщень — 0. Гілка контенту
  `panel/content` — розділ 29.
- **Попередня контрольна точка (2026-09-05, після merge PR #17–#22):**
  `HEAD = origin/main = 47fbfb88fc40103368dd692c02c3638f06e9d1f6`
  (squash-коміт PR #21, `fix: make the "book a consultation" link work in
  the gallery project modal`), активна гілка `main`, робоче дерево чисте.
  CI — `success` для кожного з PR #17–#22 (штатний `Verify` check проти
  `main`; для PR #19 додатково підтверджено штатним `pull_request`-запуском
  після узгодження з `main`). Vercel Production deployment — `success`
  після кожного merge, домен `dream-car-vavd.com` — HTTP 200, підтверджено
  реальним переглядом (маска логотипу, усі три CTA — сервісний,
  автомобільний, галерейний — uk/ru/en, мобільний viewport). Повний доказ —
  розділ 21 (PR #17–#20); розділ 22 (PR #21).
- **Попередня контрольна точка (2026-09-03, після merge PR #15):**
  `origin/main = 4588d691ab312966d9068925e8ea5b994f0f0fd8` (squash-коміт
  PR #15, `perf: reduce hero image transfer size`, злито
  `2026-09-03T21:06:16Z`); CI run `33806096697`, `success`, 93 тести;
  Vercel Production deployment `6252896057` — `success`/`Current`.
  Хронологію PR #8–#15 див. розділ 16.
...
3. Окремо — кешування та вага відеофайлів у `public/` (розділ 19, блок B) —
   **відкрито**. Відео як напрям розділено: **YouTube primary** — реалізовано,
   helper перевірено на Preview PR №26 (розділ 25); **власний
   hosted-file/Blob** — код збережений, Blob не підключено. PR №26 злито в
   `main` `2026-09-22` (розділ 27); на Production Blob за наявними записами не
   підключено, тому завантаження відео через панель там не працює; повний
   e2e YouTube-відео на сторінці авто не перевірявся.
...
**Стан на 2026-09-26:** PR #30, #31 і #26 злиті в `main`
(`71d2d28ed9cc982787dc647acb5b9661efdd9c35`, розділ 27); панель запущено в
Production, вхід через GitHub працює (розділ 28); повний hosted-цикл
публікації через `panel/content` пройдено, тестові матеріали прибрано
(розділ 29). Немає затвердженої активної технічної задачі.

**Наступний окремий етап (бізнес):** перша реальна картка «Акція або
новина» за процесом UK/EN/RU → чернетка в `panel/content` → перевірка на
Preview → ручне рішення власника про публікацію. Опубліковане в
`panel/content` потрапляє на сайт лише після перенесення в `main` (ручний PR
або активація Б4 — `docs/PANEL-hosting-and-approvals.md` §1.5) — окреме
рішення власника.

Нижче в цьому розділі — стан на момент попередніх записів (до merge
PR #26); він історичний, крім переліку «Фактично незавершені питання».

**Активної технічної задачі немає. Поточну версію сайту прийнято власником
із погодженим наповненням** (2026-09-05, розділ 23) — це не твердження про
відсутність усіх можливих помилок і не завершення майбутнього розвитку.

Останній завершений етап на Production — PR #17–#21 + PR #24 (легша
CSS-маска логотипу, CTA "консультація" в модалці послуги, передача обраного
авто у форму, той самий CTA в модалці галереї, надійне повернення фокуса на
картку галереї після закриття модалки, документація #20/#22/#23), усі
змерджені й перевірені на Production `2026-09-05`.

Повний доказ CI/deployment/production-перевірки — розділ 21 (PR #17–#20),
розділ 22 (PR #21), розділ 23 (PR #24 + приймання).

**Наступний окремий етап (історично — на той момент у роботі на гілці
`codex/admin-panel-spike`, PR #26 draft; злито `2026-09-22`, розділ 27):**
панель керування контентом. Поточний стан,
технічні рішення й наступні кроки — `PROJECT_PROGRESS.md` (розділ
«Панель…») і розділ 24 цього файла (поточна контрольна точка) та `docs/PANEL*.md`.

**Контрольна точка панелі до merge — розділ 24 (21.09.2026, автоматична
WebP-оптимізація фото; code checkpoint
`834c497be29adc39d96982f757081f5d8464d567`; на той момент PR #26 був
Draft/OPEN, `main` і Production не змінені; історично).**

**Попередня контрольна точка — 16.09.2026 (історична; факти нижче лишаються
чинними для свого моменту):** `ffa6ee707a21dd38b5bb1f5352e2a5ce1ebb837d`
...
(злито `2026-09-22`). GitHub CI (`Verify`) і Vercel Preview — успішні на кожному з цих
...
Деталі й докази — `PROJECT_PROGRESS.md` (розділ «Панель…», блок із
заголовком «АКТУАЛЬНА КОНТРОЛЬНА ТОЧКА — 16.09.2026» — це назва блоку в тому
файлі; станом на 21.09.2026 він історичний, поточна точка — розділ 24 тут).

Початковий проєкт і оцінка у зовнішньому документі
`DREAM.CAR.VAVD-mask-optimization-20260905/report/32-…`. Потребувала рішень
власника до старту (публікація напряму/через PR; поле «статус авто»;
6 карток «Галерея N» із текстом-заглушкою) — ці рішення історичні, стосуються
самого старту роботи над панеллю, не поточного стану.

Впорядкований backlog (не блокує роботу сайту) — розділ 19.
...
A1 завершено (PR #17, розділ 21). Для решти (A2–A5): по одній ізольованій
зміні з A/B-виміром медіан (3+3 або 5+5, не одиничний прогін), без
повторного аудиту Hero чи A1, без численних PageSpeed-прогонів:

| # | Кандидат | Гіпотеза | Ризик |
|---|---|---|---|
| ~~A1~~ | ~~Сирий `dream-car-logo.png` ~197 КБ (CSS `mask-image`, shimmer)~~ | **Завершено PR #17** (`2026-09-05`, squash `448acd4`) — замінено на легкий alpha-only `dream-logo-mask.png` (17 705 B, ~91% менше). Доказ перевірки — розділ 21. | — |
...
| # | Дія | Пріоритет |
|---|---|---|
| B1 | **Аудит завершено 22.09.2026, видалення не виконувалось** (read-only, деталі — звіти PANEL-LEGACY-ASSET-REFERENCE-AUDIT-01). Класифікація 8 кандидатів: 2 `RUNTIME_ACTIVE` (`cars-for-sale-gallery-logo.png`, `cars-for-sale-car.png`) — залишити; 2 `DOCS_REFERENCE_ASSET` (`reference-banner.png`, `hero-cars.png`) — залишити; 4 `RUNTIME_UNREFERENCED_BUT_DOCS_REFERENCED` (`hero-cars-old-backup.png`, `hero-cars-logo-updated.png`, `dream-car-logo-old-backup.png`, `hero-cars-logo-final.png`) — потенційне звільнення 6,35 MiB, видалення **очікує окремого дозволу власника** | середній (гігієна, швидший clone/білд) |
| B2 | Відео walkaround (`dacia-walkaround.mp4` ~49 МБ + `12-walkaround.mp4` ~7 МБ у `public/`) — стиснути або винести на зовнішній хостинг. **Стан:** YouTube (Public/Unlisted) — основний спосіб, реалізований у PR №26 (злито `2026-09-22`, розділ 27) і перевірений на Preview (helper); власний hosted-file/Blob — код є, Blob на Production не підключено; повний e2e YouTube-відео на сторінці авто не перевірявся; файли в `public/` не змінювались | **високий**, якщо відео показується на сайті |
| B3 | Фото авто в продажу та інші CMS-фото — аудит завершено 21.09.2026: 98 JPEG, максимальна сторона 2600 px, 32 файли понад 800 КБ, orphan-файлів 0. Batch-перетворення старих фото свідомо відхилене; браузерний WebP-оптимізатор використовується для нових завантажень. Деталі — `docs/PANEL-existing-photo-optimization.md` (§26) | **закрито — дій не потрібно** |
...
| ~~D4~~ | **Завершено** цим документаційним PR — секція «Зображення» в `README.md` оновлена: точний hero-файл, розрізнення альбомів/загальних фото/невикористаних файлів на диску. |
...
## 21. PR #17–#20 — злиті й підтверджені на Production (2026-09-05)

Усі чотири PR пройшли review, злиті через **Squash and merge**, кожен
required CI-check (`Verify (TypeScript, ESLint, tests, build)`) —
`success`, кожен Vercel Production deployment — `success`.
...
| [#20](https://github.com/DreamCar-vavd/DREAM.CAR.VAVD/pull/20) | `docs: record PR #17/#18/#19 merge, CI, deployment and production verification` | `096eb582030c6b628d00910c3441013d228f03e2` |

Фінальний `main`/Production SHA: **`096eb582030c6b628d00910c3441013d228f03e2`**.
...
**Підтвердження власника:** «ПРОВІРИВ ВСЕ ТЕ ПРАЦЮЄ. 17-18-19 ПРАЦЮЄ»
(дослівно, з чату, 2026-09-05). Це підтвердження перегляду **Preview**
власником — конкретний браузер у повідомленні не вказано. **Не є**
окремим ручним підтвердженням саме Production (те виконано агентом,
доказ вище) і **не є** підтвердженням доставки листів через форму (форма
на Production перевірялась лише в попередніх, не пов'язаних із цими
чотирма PR задачах — див. "DREAM.CAR.VAVD contact form live").
...
## 22. PR #21 — виправлення CTA "консультація" в галереї (2026-09-05)

**Squash-комміт у `main`:** `47fbfb88fc40103368dd692c02c3638f06e9d1f6`
(злито `2026-09-05T13:13:36Z`). Required CI — `success`. Vercel Production
deployment — `success`.

**Дефект:** посилання "Замовити консультацію" в `GalleryProjectModal.tsx`
не мало жодного обробника кліку — перехід на `#contacts` фактично
відбувався (Next.js `<Link>` коректно перехоплював клік), але модалка
ніколи не отримувала команди закритися, тому її повноекранний оверлей і
`scroll-lock` лишались активними поверх усього. Візуально виглядало як
"нічого не відбувається". Стосувалось усіх 8 карток галереї (2 реальні
фотоальбоми + 6 загальних фото), а не лише названих власником.

**Виправлення:** той самий, уже перевірений підхід сервісного/
автомобільного CTA (`isModifiedClick` + `closeForNavigation`, без нової
бібліотеки); `focusContactsHeading` винесено у спільний
`src/lib/focusContactsHeading.ts`. Escape/X/фон — не змінювались.

**Підтвердження власника:** кнопка працює на Preview PR #21 — це
підтвердження поведінки кнопки; конкретний браузер у повідомленні не
вказано.

**Перевірено на Preview і після merge — на Production
(`https://dream-car-vavd.com`):**
- Усі 8 карток галереї українською — модалка закривається, `#contacts`
  правильної локалі, фокус на заголовку "Контакти", `scroll-lock` знято.
- ru/en — по одному альбому й одній загальній картці.
- Desktop, 375px, 390px; повторний клік; CTA коли URL вже містить
  `#contacts`; раніше введені дані форми зберігаються.
- Ctrl-клік — модалка лишається відкритою, нічого не закривається/не
  пишеться.
- Сервісний і автомобільний CTA — підтверджено не зламані. Автозаповнення
  з Галереї в форму — свідомо не додавалось.
- Нових помилок консолі/CSP на Production не виявлено.

**Escape/X/фон — досліджено окремо, порівняно на Production (до PR) і на
Preview (з PR), той самий незмінений код `closeModal`:**
- Модалка коректно закривається і `scroll-lock` знімається — однаково в
  обох випадках.
- Фокус після закриття **не повертається на тригер-картку** — лишається
  на `document.body`. Підтверджено **однаково на Production і на Preview**
  — тобто це наявна, не пов'язана з PR #21 характеристика (код
  `closeModal`/`triggerRefs` цей PR не змінював). Реальне натискання
  Escape в інструментах автоматизації цієї сесії спрацьовувало не з першої
  спроби (не з кожного разу реєструвався keydown) — те саме спостерігалось
  і на Production, тобто це обмеження середовища перевірки, а не коду.
- **Не є регресією PR #21**, не блокувало його merge. **Виправлено окремо
  в PR #24** — див. розділ 23.

## 23. PR #24 — надійне повернення фокуса на картку галереї + приймання версії (2026-09-05)

**Squash-комміт у `main`:** `8e262cd3166a32dffee49e99002fa11f59628ce3`
(злито `2026-09-05T19:46:50Z`). Required CI `Verify (TypeScript, ESLint,
tests, build)` — **`success`** для head `665d88dc37606445d540c50ca92dbd233db5358d`.
Файли: лише `src/components/GalleryGrid.tsx`.
**Git SHA → deployment:** GitHub Production deployment `6285085216`
(ref = sha = `8e262cd3…`) → стан `success` → домен `dream-car-vavd.com`.

**Дефект:** після `картка → модалка → Escape / X / фон` фокус лишався на
`document.body`, а не на картці, що відкрила модалку.
**Причина (з доказом):** `closeModal` відкладав `.focus()` через
`requestAnimationFrame`, а браузер призупиняє rAF-колбеки у фоновій
вкладці — прямий тест: за `document.hidden === true` rAF-колбек не
викликається взагалі (`setTimeout` — викликається). Друга причина —
`triggerRefs` як Record з inline callback-ref'ами, що тимчасово стають
`null` при перемиканні модалки.
**Виправлення:** один `lastTriggerRef` (елемент захоплюється на кліку через
`openModal(id, event.currentTarget)` — патерн `ServicesGrid`) +
`window.setTimeout(() => …, 0)` замість rAF (патерн
`CarListingGallery.close()`) + `trigger?.isConnected` guard. Кнопка
«Замовити консультацію» як і раніше **не** повертає фокус на картку.
Жодних нових залежностей, один файл.

**Перевірено на Production (`https://dream-car-vavd.com`) після deployment:**
- Картка Maserati/Volvo/«Галерея N» → Escape / X / фон → модалка
  закривається, `scroll-lock` знято, **фокус повертається на ту саму
  картку**. uk/ru/en. Desktop і 375px.
- «Замовити консультацію» → `#contacts` правильної локалі, фокус на
  `#contacts-heading`, **без стрибка назад до Галереї** (перевірено з
  очікуванням +1.1 с).
- Сервісний CTA → `car-service`; автомобільний CTA → `Dacia Sandero`.
- Нових помилок застосунку в консолі не виявлено.
- Обмеження перевірки: апаратне натискання клавіш (`computer`-інструмент)
  для Escape/Enter у цій сесії ненадійне — використовувались справжні
  `KeyboardEvent` крізь наявний слухач `useDialogFocusTrap` → `onClose` та
  справжні DOM-кліки для X/фону/CTA. Реальна заявка не надсилалась.

**Підтвердження власника (2026-09-05, з чату):**
1. Перевірка повернення фокуса працює.
2. Бізнес-дані залишаємо без змін.
3. Галерею та її поточне наповнення залишаємо як є.
4. Власник отримує заявки на електронну пошту.

Власник не називав конкретного браузера, дати контрольної заявки чи
технічного сценарію — тут вони й не приписуються.

**→ Поточну версію сайту прийнято власником із погодженим наповненням.**
Це не твердження про відсутність усіх можливих помилок і не завершення
майбутнього розвитку.

**Фактичні незакриті обмеження (не блокують прийняту версію):**
- Реальний Safari (десктоп / iOS) — не перевірявся ні власником (браузер у
  його підтвердженнях не названо), ні агентом.
- Автоматизований DOM-тест фокуса — неможливий (у проєкті немає jsdom /
  Testing Library; додавати залежність не дозволено). Регресійне покриття —
  ручна браузерна перевірка.
- `ServicesGrid.tsx` має ідентичний `requestAnimationFrame(() =>
  trigger.focus())` — та сама прихована rAF-характеристика (фокус не
  повертається на картку послуги, якщо вкладка у фоні на момент закриття).
  Свідомо **не** чіпалось у цьому вузькому PR; окремий однорядковий
  кандидат (rAF → `setTimeout`).
- 6 карток «Галерея N» відкривають модалку з текстом-заглушкою
...
- Панель керування контентом — на момент запису в роботі на гілці
  ...
  змінює) — деталі в `PROJECT_PROGRESS.md`, блок із заголовком
  «АКТУАЛЬНА КОНТРОЛЬНА ТОЧКА — 16.09.2026» (станом на 21.09.2026 історичний).

## 24. Панель — контрольна точка 21.09.2026: автоматична WebP-оптимізація фото

> **Історичний запис.** PR #26 злито в `main` `2026-09-22` (розділ 27):
> описаний нижче оптимізатор входить у Production-збірку, але в Production
> наживо окремо не перевірявся. Статуси «Draft/OPEN» і «недоступна в
> Production» нижче — стан на 21.09.2026.

Реалізовано на гілці `codex/admin-panel-spike` (PR #26 — **Draft, OPEN, не
змерджено**). Code checkpoint: `834c497be29adc39d96982f757081f5d8464d567`
(поточний head PR — дивитись у GitHub/Git; кожен docs-коміт його змінює).
`main` і Production **не змінені**; функція **недоступна в Production** —
перевірена лише на Vercel Preview.

Що зроблено: усі чотири фото-поля Keystatic (автомобілі, галерея, послуги,
банери/акції/новини) стискають вибране фото у браузері до WebP ще до запису в
робочу копію (макс. вхід 12 МБ, макс. сторона 2400 px, без збільшення малих
фото, збереження прозорості PNG та EXIF-повороту). Серверного API немає
(ліміт тіла запиту Vercel Function ~4,5 МБ не підходить для 12-МБ файлів).

Перевірки:
- 23/23 вузькі тести оптимізатора; TypeScript, ESLint, `npm run build`, CI
  (Verify) і Vercel Preview — успішні.
- Preview (авторизований Keystatic, нічого не збережено): JPEG 3000×2000 →
  WebP 2400×1600; файл 12 МБ + 1 байт відхилено з поясненням, попереднє
  значення поля не замінено; PNG 800×600 не збільшено, alpha збережено
  (кут 0, центр 255); EXIF Orientation 6 → 800×1200 (один правильний
  поворот); race-сценарій (новіший вибір перемагає старіший) пройдено.
- Тестові файли створювались поза репозиторієм і видалені; у контент нічого не
  збережено.

Не заявляється: гарантоване видалення EXIF/GPS; 200–800 КБ — лише
рекомендація. Чинні обмеження PR #26 (Blob не підключено, Production Neon не
підключено, рішення про приватність завантажених відео, publication workflow у
`main` не активовано, hosted acceptance 20/22) — без змін.

## 25. Панель — контрольна точка 21.09.2026: YouTube як основний спосіб відео

> **Історичний запис.** PR #26 злито в `main` `2026-09-22` (розділ 27);
> YouTube-helper входить у Production-збірку, але в Production наживо
> окремо не перевірявся. Статуси «Draft/OPEN» нижче — стан на 21.09.2026.

Гілка `codex/admin-panel-spike`, PR #26 — **Draft, OPEN, не змерджено**;
`main` і Production **не змінені**, функції на Production **немає** (перевірено
лише на Vercel Preview).

Коміти:
- основна реалізація: `ea9a21ad5d445b0991cb172dc68822d2ffab0907`;
- остаточний code-fix checkpoint: `bf85e0a1b2e593c6b7028fa9a35b7d97ca19ea6a`
  (порожнє `external-link` блокує публікацію; вузьке визначення YouTube-домену);
- docs-fix: `2d76b3b85df95e115e0d36df60e6acb9f3652720`.
Поточний head PR — дивитись у GitHub/Git (кожен docs-коміт його змінює).

Що перевірено:
- **20/20** вузьких тестів YouTube/`carVideo`.
- **Live Preview, helper `/panel/video`** (авторизований Chrome): валідні
  формати `watch`, `youtu.be`, `/shorts/`, `/embed/` нормалізуються до
  `https://www.youtube-nocookie.com/embed/<id>`; підроблений домен, короткий ID
  і `http://` відхиляються з поясненням, iframe не з'являється; Blob не
  підключено — секція YouTube при цьому видима; помилок консолі немає.
- Save/Publish у цих перевірках **не виконувались**; у картки нічого не
  вводилось.

**Не перевірено наживо:** повний end-to-end — YouTube-URL, реально збережений у
картці автомобіля → відкриття цього відео на сторінці автомобіля. Тому функцію
не можна вважати повністю end-to-end прийнятою; підтверджено helper/preview і
код+тести.

Власний hosted-file/Blob: код збережений, Blob **не підключено**; рішення про
`public`/`private` для завантажених відео лишається за власником.

## 26. Старі CMS-фото — рішення 21.09.2026

Аудит старих фото під `public/images/cms/**` **завершено** (read-only проєкція,
нічого не змінювалось, візуальна якість не порівнювалась): 98 JPEG, 59,35 MiB
фізично (≈29,54 MiB унікально), WebP q84 дав би ≈6,25 MiB фізичної / ≈3,01 MiB
унікальної економії (≈10 %); 16 файлів стали б більшими. Повні цифри й топ-10 —
`docs/PANEL-existing-photo-optimization.md`.

**Рішення:** batch-перетворення старих фото **свідомо відхилене** (це не
невиконаний борг); JSON, `published.json`, working-медіа й `_pub/` не
переписуються; браузерний оптимізатор — лише для нових завантажень. Повернутись
лише за суттєвого зростання контенту або реального обмеження deployment.

Міграція двох MP4 на YouTube (≈54,7 MiB) — більший потенційний виграш, але
**не виконана**: чекає на два URL від власника.

## 27. PR #30, #31 і #26 — злиті в `main` (16–22.09.2026)

Усі три злиті через **Squash and merge**; required CI-check `Verify` на
`main` після кожного merge — `success`.

| PR | Зміст | Squash-коміт | Злито (UTC) | CI на `main` |
|---|---|---|---|---|
| #30 | Постійне Telegram-посилання `https://t.me/DREAM_CAR_VAVD` замість env-змінної | `e143b94ecf0ab09a0aff9c6d2e90c576f958860a` | `2026-09-16T12:50:45Z` | run `35098211263` — `success` |
| #31 | Безпека залежностей: `next` 16.3.5, пропатчені транзитивні `sharp` і `js-yaml` | `a1b2fce8b082526e1b3119e4338e83826719990d` | `2026-09-22T13:37:59Z` | run `35734749277` — `success` |
| #26 | Панель керування контентом: авто, галерея, послуги, банери/акції/новини, контакти; uk/ru/en; чернетки й draft-preview; перевірка прав на кожному запиті; WebP-оптимізатор фото; YouTube-відео | `71d2d28ed9cc982787dc647acb5b9661efdd9c35` | `2026-09-22T16:38:56Z` | run `35755509230` — `success` |

- PR #31 злито першим; конфлікт PR #26 з ним (`package.json`/lockfile)
  розв'язано перед merge — `next` = `16.3.5`, залежності панелі збережені.
- Dependabot PR #27/#28/#29 закриті як перекриті PR #31; відкритих
  Dependabot-сповіщень станом на `2026-09-26` — 0.
- Разом із PR #26 у `main` потрапив код Б4 (`scripts/publish-content.ts` і
  його тести), але workflow **не активовано**: `content-guard.yml` лежить у
  `.github/workflows-proposed/`; активний лише `.github/workflows/ci.yml`
  (push і PR у `main`).

## 28. Панель у Production — запуск і вхід (22–26.09.2026)

- **Гілка контенту `panel/content`** створена `2026-09-22` на `71d2d28…`
  (через GitHub API). Production-панель пише саме в неї, не в `main`.
- **Production-змінні у Vercel** (додав власник; тут — лише назви):
  `NEXT_PUBLIC_KEYSTATIC_STORAGE_KIND`, `KEYSTATIC_GITHUB_REPO_OWNER`,
  `KEYSTATIC_GITHUB_REPO_NAME`, `NEXT_PUBLIC_KEYSTATIC_GITHUB_APP_SLUG`,
  `PANEL_CONTENT_BRANCH`, `KEYSTATIC_GITHUB_CLIENT_ID`,
  `KEYSTATIC_GITHUB_CLIENT_SECRET`, `KEYSTATIC_SECRET`.
- **Діагностика входу:**
  1. OAuth 404 — у `KEYSTATIC_GITHUB_CLIENT_ID` було вставлено символ `⌘V`
     замість значення; власник виправив.
  2. Один Production-білд упав на `next/font/google` (Turbopack) —
     тимчасовий збій: чиста ізольована збірка того самого SHA пройшла,
     наступний деплой — `READY`.
  3. HTTP 500 після обміну OAuth-коду — точна причина з Runtime Logs
     власника: `KEYSTATIC_SECRET must be at least 32 characters long`.
     Власник замінив `KEYSTATIC_SECRET` на випадкове значення з 64 символів.
     `KEYSTATIC_GITHUB_CLIENT_SECRET` також оновлено під час діагностики, але
     причиною 500 він не був.
- **`2026-09-25` — вхід через GitHub працює** (скріншот власника):
  `DreamCar-vavd`, дашборд — 3 авто, 8 робіт галереї, 5 послуг.
- **Client secrets GitHub App (`2026-09-25`):** read-only preflight (лише
...
- **Перевірка гілки (`2026-09-26`) — PASS:** `/panel` показує «Робоча
  гілка: `panel/content`» (скріншот власника); посилання «Keystatic» з
  `/panel` відкриває `/keystatic/branch/panel%2Fcontent`. Гілку визначає
  `resolveContentBranch` (`src/lib/content/store/branch.ts`) як
  `PANEL_CONTENT_BRANCH || VERCEL_GIT_COMMIT_REF`. **Висновок, не
  перевірений факт:** значення, найімовірніше, походить із Production
  `PANEL_CONTENT_BRANCH=panel/content` (для деплою, зібраного з `main`,
  `VERCEL_GIT_COMMIT_REF` дорівнював би `main`); точне джерело без перевірки
  Vercel env або метаданих деплою не підтверджене.
- **Правило для редактора:** заходити в Keystatic тільки через посилання з
  `/panel`. Прямий `/keystatic` (зокрема після повторного входу) відкриває
  гілку за замовчуванням — `main`; `main` захищена (розділ 3).
- **Не підключено в Production (за наявними записами):** база заявок
  (`LEADS_DATABASE_URL`, Б3-prod) — `/panel/leads` показує «не налаштовано»,
  заявки надходять лише на пошту через Formspree; Vercel Blob (Б2) —
  завантаження відео через панель не працює; Б4 — автоматичне перенесення
  `panel/content` → `main`.

## 29. Панель — повний hosted-цикл публікації (26.09.2026)

Контрольний тест у `panel/content` з окремого дозволу власника; `main`,
Production і конфігурація не змінювались. Тестова картка — колекція
«Банери, акції, новини» (`src/content/cms/promos/`): ID
`test-publikatsiyi-20260926`, порядок `9999`, заголовки: UK
«ТЕСТ ПУБЛІКАЦІЇ — НЕ ПУБЛІКУВАТИ», EN «PUBLICATION TEST — DO NOT PUBLISH»,
RU «ТЕСТ ПУБЛИКАЦИИ — НЕ ПУБЛИКОВАТЬ».

**Пройдено** (дії власника; візуальні перевірки — його скріншоти):
1. Створення картки в Keystatic на `panel/content`; перевірено ID і slug.
2. UK, EN і RU окремо позначено перевіреними.
3. Перегляд чернетки трьома мовами.
4. Публікація через `/panel` → картку видно на Preview гілки
   `panel/content`.
5. «Прибрати з сайту» → на новому Preview картки немає; розділ «Акції та
   новини» зник повністю (картка була єдиною) — очікувана поведінка.
6. Видалення робочої картки в Keystatic (`Delete entry → Yes, delete`) →
   колекція «Банери, акції, новини» порожня.
7. `/panel` → «Завершити видалення (1)» — прибрано UK/EN/RU review-записи.
8. Фінал `/panel`: «Неопублікованих змін немає — сайт відповідає робочим
   карткам»; блок «Незавершені видалення» зник.

Попередню тестову чернетку `test-ne-publikuvaty-20260926` також видалено —
вона лишається лише в історії комітів.

**Git-докази (перевірено агентом через GitHub API, `2026-09-26`):**
- `panel/content` = `4017c53286cc48a672353bdb9d5eabf0c0ca453a` — на 10 комітів
  попереду `main`, 0 позаду. Усі 10 — коміти тесту (`582006a` … `4017c53`,
  `2026-09-26T07:03Z`–`16:21Z` UTC), серед них публікація `905df45` і зняття
  з публікації `67bbf8e`.
- Підсумкова різниця `main...panel/content` — **один файл, один рядок**:
  `src/content/cms/published.json`, поле `publishedAt`
  `2026-09-14T09:31:54.915Z` → `2026-09-26T15:01:22.107Z`. Інших змін
  контенту, медіа чи коду немає.
- У дереві `panel/content` немає шляхів із `test-publikatsiyi-20260926` чи
  `test-ne-publikuvaty-20260926`; у `published.json` і `review-state.json`
  немає тестових ID і текстів.
- Preview-збірки Vercel для `905df45` (запис GitHub Deployments `6680162439`)
  і `67bbf8e` (`6680384880`) — `success`.
- `main` під час тесту — `71d2d28…`, без змін. Коміти в `panel/content` не запускають CI
  (`ci.yml` — лише push/PR у `main`), але кожен запускає Preview-збірку
  Vercel.

**Межі:** merge, зміни `main`, Production redeploy, DNS, Vercel env, токени,
секрети й тарифи — не виконувались.

**Виявлені моменти** (потребують окремої вузької діагностики, не
приписуються як дефекти):
- на картці в `/panel` не з'явилось посилання «Редагувати в Keystatic →»,
  хоча в коді воно передбачене умовно (`row.editHref`);
- після «Прибрати з сайту» робоча картка зберігала `visible: true` і кнопку
  «Опублікувати» — відповідає моделі робочої чернетки, але може плутати;
- після фінального очищення панель могла показувати відсутність деплою для
  поточного службового коміту — не блокер (зняття з публікації окремо
  перевірено на Preview);
- для будь-якої гілки, крім `main`, `/panel` підписує її «(тестова гілка —
  не Production…)» — для `panel/content` у Production це неточно.

Наступний бізнес-етап — розділ 15.
