...
Поточна контрольна точка: **`096eb582030c6b628d00910c3441013d228f03e2`** (squash-коміт
документаційного PR #20, злито `2026-09-05T11:55:34Z`, після PR #17
(легша CSS-маска логотипу), PR #18 (CTA "консультація" в модалці послуги)
і PR #19 (передача обраного авто у форму) — усі три `2026-09-05`, доказ —
розділ 21).
...
- Стара папка `/Users/apple/Desktop/MY PROEKT-2` є резервною історичною
  ...
  дозволу власника.
- **Поточна підтверджена контрольна точка (2026-09-05, після merge PR #17–#20):**
  `HEAD = origin/main = 096eb582030c6b628d00910c3441013d228f03e2`
  (squash-коміт документаційного PR #20), активна гілка `main`, робоче
  дерево чисте. CI — `success` для кожного з PR #17/#18/#19/#20 (штатний
  `Verify` check проти `main`; для PR #19 додатково підтверджено штатним
  `pull_request`-запуском після узгодження з `main`). Vercel Production
  deployment — `success` після кожного merge, домен `dream-car-vavd.com` —
  HTTP 200, підтверджено реальним переглядом (маска логотипу, обидва CTA,
  uk/ru/en, мобільний viewport). Повний доказ — розділ 21.
...
Останній завершений етап на Production — PR #17–#20 (легша CSS-маска
логотипу, CTA "консультація" в модалці послуги, передача обраного авто у
форму, документація), змерджені й перевірені `2026-09-05`. Власник
підтвердив у чаті: «ПРОВІРИВ ВСЕ ТЕ ПРАЦЮЄ. 17-18-19 ПРАЦЮЄ» — це
підтвердження перевірки на Vercel Preview (браузер у повідомленні не
вказано), не окрема перевірка самого Production і не підтвердження
доставки листів через форму. Доказ CI/deployment/production-перевірки —
розділ 21.

**Активний, ще не змерджений PR:** [#21](https://github.com/DreamCar-vavd/DREAM.CAR.VAVD/pull/21)
(draft) — виправлення непрацюючої кнопки "Замовити консультацію" в модалці
галерейного проєкту (усі 8 карток: 2 реальні альбоми + 6 загальних фото).
Дефект підтверджений власником і відтворений на Production перед фіксом;
сам фікс перебуває на Preview, очікує перегляду власником і окремого
дозволу на merge — **не вважати виправленим на Production**, доки не
змерджено. Деталі — розділ 21.
