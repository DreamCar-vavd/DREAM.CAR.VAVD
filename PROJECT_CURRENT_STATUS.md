...
Поточна контрольна точка: **`8e262cd3166a32dffee49e99002fa11f59628ce3`**
...
- Стара папка `/Users/apple/Desktop/MY PROEKT-2` є резервною історичною
  ...
  дозволу власника.
- **Поточна підтверджена контрольна точка (2026-09-05, після merge PR #17–#22):**
...
3. Окремо — кешування та вага відеофайлів у `public/` (розділ 19, блок B) —
   **відкрито**. Відео як напрям розділено: **YouTube primary** — реалізовано,
   ...
   hosted-file/Blob** — код збережений, Blob не підключено; функцій PR №26 на
   Production ще немає.
...
**Наступний окремий етап (у роботі на гілці `codex/admin-panel-spike`,
PR #26 draft, не змерджено):** панель керування контентом. Поточний стан,
...
**Поточна контрольна точка панелі — розділ 24 (21.09.2026, автоматична
WebP-оптимізація фото; code checkpoint
`834c497be29adc39d96982f757081f5d8464d567`; PR #26 лишається
Draft/OPEN, не merged; `main` і Production не змінені).**
...
| B2 | Відео walkaround (`dacia-walkaround.mp4` ~49 МБ + `12-walkaround.mp4` ~7 МБ у `public/`) — стиснути або винести на зовнішній хостинг. **Стан:** YouTube (Public/Unlisted) — основний спосіб, реалізований у PR №26 (Draft) і перевірений на Preview (helper); власний hosted-file/Blob — код є, Blob не підключено; **на Production цього ще немає**, файли в `public/` не змінювались | **високий**, якщо відео показується на сайті |
...
- 6 карток «Галерея N» відкривають модалку з текстом-заглушкою
  ...
  залишено **без змін**.
- Панель керування контентом — у роботі на гілці `codex/admin-panel-spike`
  (PR #26, draft, **не змерджено** в `main`). Стан і handoff:
