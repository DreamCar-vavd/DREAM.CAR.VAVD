...
| Токен для пушу | **`CONTENT_PUBLISH_TOKEN`** — окремий, вузько-scoped секрет (`contents:write` на цей репозиторій, нічого більше), що лежить лише в Environment `content-publish`, **не** типовий `GITHUB_TOKEN`. Це репозиторний секрет, який ця сесія не створювала — Environment ще не існує, job фізично не може запуститись, доки власник його не створить. |
...
2. Repo → Settings → Environments → створити **`content-publish`**:
   - Deployment branch and tag rules → **Protected branches** (або явно
     лише `main`) — це те, що GitHub звіряє з `ref` запуску ПЕРЕД видачею
     секрету;
