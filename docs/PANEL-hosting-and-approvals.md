...
| Який check перевіряє дані | `scripts/publish-content.ts` (регресійні тести — `scripts/publish-content.test.ts`, реальні тимчасові git-репозиторії, не мок): читає лише через `git diff`/`git ls-tree`/`git cat-file`/`git archive` (жодних робочих-дерево операцій), рахує diff від merge-base до `content_sha`, **валить**, якщо він торкається будь-чого, крім `src/content/cms/published.json`, `src/content/cms/review-state.json`, `public/images/cms/**`; далі запускає `npm run content:guard` **двічі** — на знімку самого `content_sha` (валідність) і окремо на ФІНАЛЬНОМУ складеному дереві, яке буде опубліковано (цілісність — саме тут ловиться випадок «видалили фото, яке `main` вже почав використовувати іншим комітом»). Відсутній/непрочитний `published.json` — відмова, не публікація порожніх даних. `content_sha` звіряється як повний 40-символьний hex і як реальний закомічений об'єкт-коміт до будь-якого використання. |
