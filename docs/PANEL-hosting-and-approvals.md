...
**⚠️ Виправлено двічі — 15.09.2026 (ранок) і 15.09.2026 (після незалежної
перевірки Codex):**
1. Перша версія цієї таблиці стверджувала, що гілка контенту не може
   запустити свою копію workflow, бо GitHub на `push` бере визначення з
   `main`. Це неправда для `push`: GitHub виконує САМЕ ТУ версію workflow-
   файлу, що лежить у запушеному коміті
   ([Events that trigger workflows](https://docs.github.com/en/actions/reference/events-that-trigger-workflows) —
   `push`: `GITHUB_SHA` = «Tip commit pushed to the ref»). Виправлено
   заміною тригера на `workflow_dispatch`.
2. **Друга правка теж була занадто категоричною й далі неправильною** —
   Codex слушно вказав: твердження «`workflow_dispatch` завжди виконує YAML
   з `main`, незалежно від `ref`» перевірити не вдалося. Офіційна
   документація GitHub однозначно каже лише, що файл має **існувати** в
   `main`, щоб бути диспетчеризованим узагалі — але мовчить про те, чию
   саме копію (`main` чи вказаного `ref`) GitHub виконує ПІСЛЯ диспетчеризації;
   джерела спільноти з цього приводу суперечать одне одному. Замість
   покладатися на цю нез'ясовану властивість, схему перебудовано так, щоб
   вона взагалі від неї не залежала — див. нижче.

**Актуальний захист — GitHub Environment, а не сам тригер.** Незалежно від
того, яка копія YAML фактично виконується, право писати в `main` має
залишатись недосяжним для запуску, чий `ref` — гілка контенту. Це
забезпечує GitHub Environment із **deployment branch policy**, обмеженою
лише `main`: секрет у такому Environment GitHub видає (чи не видає) на
основі `ref` самого запуску, звіряючи це ДО виконання будь-яких кроків —
підміна вмісту YAML тут нічого не дає, бо саме визначення того, кому
видавати секрет, контролюється GitHub, а не файлом у репозиторії.
...
| Що запускає перевірку/злиття | Workflow `content-guard.yml`, тригер — **`workflow_dispatch`** (не `push`) із обов'язковим входом `content_sha`; job оголошує **`environment: content-publish`**. |
| Який check перевіряє дані | `scripts/publish-content.ts` (регресійні тести — `scripts/publish-content.test.ts` + `scripts/publish-content-panel-integration.test.ts`, реальні тимчасові git-репозиторії, не мок): читає лише через `git diff`/`git ls-tree`/`git cat-file`/`git archive` (жодних робочих-дерево операцій), рахує diff від merge-base до `content_sha`. **Дві різні межі, свідомо розділені (Б4 Block A):** diff'у ДОЗВОЛЕНО торкатися будь-чого під `src/content/cms/**` і `public/images/cms/**` — реальна панель комітить у гілку контенту окремо на кожне збереження Keystatic (`src/content/cms/cars/<slug>.json` тощо, робоча копія) і окремо на кожне підтвердження мови, тож інакше звичайна історія гілки після кількох дій блокувала б власну публікацію; але ПЕРЕНОСЯТЬСЯ в `main` рівно `published.json`/`review-state.json`/схвалені `content:guard`-маніфестом медіа — робоча копія колекцій ніколи не потрапляє в коміт публікації, перевірено окремим тестом. Будь-що ПОЗА цими двома директоріями (`.ts`/`.js`/`.yml`/`package.json`) — **валить**, як і раніше. Далі запускає `npm run content:guard` **двічі** — на знімку самого `content_sha` (валідність) і окремо на ФІНАЛЬНОМУ складеному дереві, яке буде опубліковано (цілісність — саме тут ловиться випадок «видалили фото, яке `main` вже почав використовувати іншим комітом»). Відсутній/непрочитний `published.json` — відмова, не публікація порожніх даних. `content_sha` звіряється як повний 40-символьний hex і як реальний закомічений об'єкт-коміт до будь-якого використання. |
| Хто/що виконує «злиття» в `main` | Той самий `publish-content.ts`, але **не через `--repo`'s робоче дерево чи індекс** — через ІЗОЛЬОВАНИЙ тимчасовий git-індекс (`GIT_INDEX_FILE`), що ніколи не торкається того, що вже застейджено/незакомічено/невідстежено в реальній копії. Компонує нове дерево (`git write-tree`) → комітить (`git commit-tree`, автор/committer — явні env-змінні, не глобальний git-конфіг) → атомарно оновлює `refs/heads/main` (`git update-ref <new> <old>` — це compare-and-swap, не «прочитати-потім-записати» з вікном для гонки) → пушить (звичайний push, ніколи не `--force`). Ідемпотентно: повторна обробка вже застосованого `content_sha` — без нового коміту. **Захист від застарілої публікації:** кожен коміт зберігає застосований `content_sha` у своєму повідомленні; новий запуск читає останній застосований SHA з історії `main` і вимагає, щоб новий `content_sha` був його git-нащадком — старіший SHA (предок) відхиляється без перезапису, розбіжна історія — як конфлікт, точний повтор — без нового коміту. |
| Токен для пушу | **`CONTENT_PUBLISH_TOKEN`** — окремий, вузько-scoped секрет (`contents:write` на цей репозиторій, нічого більше), що лежить лише в Environment `content-publish`, **не** типовий `GITHUB_TOKEN`. **Виправлено 15.09.2026 (друга незалежна перевірка):** Environment без секрету НЕ блокує запуск job — GitHub сам автоматично створює Environment із такою назвою, якщо його ще нема, без жодних правил і секретів ([GitHub docs](https://docs.github.com/en/actions/how-tos/deploy/configure-and-manage-deployments/manage-environments): «Running a workflow that references an environment that does not exist will create an environment with the referenced name»). Job **стартує**. Відмова стається інакше: крок checkout явно передає `token: ${{ secrets.CONTENT_PUBLISH_TOKEN }}`; без секрету це порожній рядок, а `token` в `actions/checkout` позначено `required: true` у самому коді дії (`core.getInput('token', {required: true})`) — явно переданий (нехай і порожній) вхід ніколи не замінюється власним `default: ${{ github.token }}` дії (той спрацьовує лише, якщо ключ `with:` узагалі не передано). Результат — явна, конкретна відмова («Input required and not supplied: token»), без тихого переходу на `GITHUB_TOKEN`. Власнику потрібні ОБИДВА кроки — і branch policy Environment, і сам секрет; наявність лише одного з двох так само провалить job. |
| Хто/що ЗАПУСКАЄ dispatch | Поки — **лише вручну**: власник/помічник із правом Write, `gh workflow run content-guard.yml -f content_sha=<sha>` або кнопка «Run workflow». Автоматичний запуск одразу після публікації в панелі — окреме, свідоме рішення власника пізніше. |
| Що запускається після пушу в `main` | Push авторизований як `CONTENT_PUBLISH_TOKEN`, не `GITHUB_TOKEN` — типовий виняток «GITHUB_TOKEN не тригерить інші workflow» тут узагалі не застосовний в той чи інший бік; чи перезапустить цей пуш CI `Verify` — залежить від природи самого токена, живо цією сесією не перевірено. `content:guard` у цьому самому job'і — довірена перевірка саме для контент-змін незалежно від цього. Vercel — інша система (власний webhook на `push`, поза механікою токенів Actions), деплой мав би спрацювати як завжди (**найімовірніше, не перевірено живо**). |
| Чому цим шляхом не проходить довільний код | Навіть якщо припустити найгірший випадок (виконується змінена копія YAML із гілки контенту) — вона все одно не отримає `CONTENT_PUBLISH_TOKEN`, бо GitHub видає Environment-секрети за `ref` запуску, звіреним із branch policy, ДО виконання будь-якого кроку. Додатково: будь-який `.ts`/`.js`/`.yml`/`package.json` у diff → `publish-content.ts` падає з кодом 2. |
| Самопогодження / небезпечний bypass | Тут немає approving review взагалі — тому немає й «сам собі Approve». Захист — **не review**, а: (1) Environment branch policy контролює видачу секрету незалежно від вмісту YAML, (2) машинний allowlist + подвійний content-guard (знімок і фінальний результат), (3) `GITHUB_TOKEN` цього job'а обмежений до `contents: read` — сам пуш ним узагалі не робиться. |
...
**Жодного з цих кроків ця сесія не виконала** — усі потребують доступу до
Settings/Secrets, який поза межами дозволу. `content-guard.yml`
(`environment: content-publish`) фізично не зможе запуститись, доки крок 3
не буде зроблено — це навмисно, не недогляд.

1. Створити окрему, вузько-scoped ідентичність для запису в `main` — GitHub
   App **лише** з правом `contents: write` на цей репозиторій (окремо від
   Keystatic-App, яким користуються редактори для входу — той має ширші
   права й належить іншим людям), або fine-grained PAT з тим самим єдиним
   правом.
2. Repo → Settings → Environments → створити **`content-publish`**:
   - Deployment branch and tag rules → **«Selected branches and tags»** →
     додати правило з іменем гілки саме **`main`**. **Не «Protected
     branches only»** — це інше налаштування: воно дозволило б БУДЬ-яку
     гілку з увімкненим branch protection, не лише `main` (якщо колись
     захист поставлять ще на якусь гілку — вона теж отримає доступ до
     секрету). «Selected branches and tags» з явним іменем `main` — єдиний
     варіант, що гарантовано звужує до однієї конкретної гілки;
   - Secrets → додати **`CONTENT_PUBLISH_TOKEN`** зі значенням токена з
     кроку 1. Не репозиторний секрет — саме секрет цього Environment.
     **Обидва підпункти обов'язкові** — Environment без жодних правил
     GitHub створює автоматично сам (без цього кроку), але без секрету
     job однаково провалиться на кроці checkout (явна відмова, не
     блокування самого запуску — див. таблицю вище).
3. Repo → Settings → Rules → Rulesets → гілка `main`:
   - Require a pull request before merging → Required approvals: **1**;
   - Require status checks: **`Verify`** (і, за бажанням, `content-guard`);
   - Require approval of the most recent reviewable push: **on**;
   - **Bypass list**: додати **саме ідентичність із кроку 1** (не
     `github-actions[bot]` — пуш тепер робить не він).
4. Змержити code-PR, що переносить `content-guard.yml` у `.github/workflows/`.
5. Перевірити: у `/panel` опублікувати дрібну зміну → у гілці `panel/content`
