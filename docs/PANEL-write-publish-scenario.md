...
5. **Перевірити залишок у мовних підтвердженнях.** `src/content/cms/review-state.json`
   у гілці не повинен містити ключа `zzz-test-panel`. Keystatic-видалення картки
   його **не чистить** (Delete entry — це прямий коміт, він не проходить через
   панель). З **П36** цим опікується сама панель: `getPanelData` ігнорує рядок
   review-стану, під яким немає робочої картки (не гейтить і не фарбує дашборд),
   виводить його в `PanelData.staleReviewSlugs`, а **наступний** запис
   review-стану — будь-яке «Позначити перевіреним» по будь-якому матеріалу —
   прибирає такі рядки тим самим комітом (`confirmLocale` → `pruneReview`). Тож
   у чистому прогоні достатньо після видалення `zzz-test-panel` один раз
   натиснути «Позначити перевіреним» на будь-якому іншому матеріалі й перечитати
   `review-state.json` за новим SHA гілки — ключа `zzz-test-panel` там уже не
   буде. Якщо такого запису до кінця прогону не сталося — прибрати ключ
   `zzz-test-panel` окремим комітом вручну (залишковий, не блокер).

   Межа П36: якщо картку **відновити** з git після того, як панель зробила
   prune, усі мови покажуть «потребує перевірки», доки їх не підтвердять знову —
   це навмисно (ніколи не показуємо хибне «перевірено»), а не регресія.
