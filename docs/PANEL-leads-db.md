...
**Статус (2026-09-11, задача 21:20): ПІДКЛЮЧЕНО й перевірено наживо на Preview.**

- Власник створив тестовий Neon-проєкт `dream-car-leads-test` (AWS Frankfurt,
  Postgres 17, compute 0.25↔2 CU) і передав owner-рядок через `.env.migrate`
  (ніколи не в чат).
- Асистент: `npm run leads:migrate` → таблиця `leads` створена (10 колонок);
  роль `leads_app` створена — **лише** `SELECT, INSERT` на `leads`, `statement_timeout='4000ms'`,
  `idle_in_transaction_session_timeout='10s'`. Звірено підключенням **як `leads_app`**:
  `UPDATE`/`DELETE`/`CREATE TABLE`/читання `pg_shadow` — усі **denied (42501)**;
  `SELECT`/`INSERT` — allowed; `statement_timeout` у сесії = `4s`.
- Синтетична перевірка через реальний адаптер (`createPgLeadsStore`, без
  надсилання форми/email): `create()` з тим самим ключем ідемпотентності →
...
- **Жива звірка на Preview:** `/panel/leads` → банер «не налаштоване» зник,
  ...
  синтетичних рядків → **«Заявок поки немає.»** (порожньо, коректно).
- `.env.migrate` видалено, буфер обміну очищено. `main` @ `ce1977af` — не
  зрушений; Production-середовище не чіпалось.

**Залишок:** Б3-prod (реальні заявки з робочого сайту) — окремий етап, потребує
коду в `main` (розділ «ЗАПИТ ВЛАСНИКУ» нижче). Питання зберігання/GDPR
(розділ 10) — до Б3-prod.
