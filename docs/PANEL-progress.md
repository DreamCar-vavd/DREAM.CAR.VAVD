...
- **Remote HEAD:** `3c25963` (docs 16:08 §1/§7) + docs 20:24 зверху. Останній
  **код** — `1f6b86f` (leads: hosted → not-configured; фікс витоку деталей
  підключення; звірка прав `leads_app`). Між ними — `91d6f81` (docs) + 13
...
- **Задача 20:24→21:20 (Б3-preview + залишки):**
  ...
  - **Блок 6 — Production-план.** Звірено `origin/main`: **немає жодного файлу
    ...
    коду, який його читає, у `main` нема. Для Б3-prod треба спершу доставити
    код у `main` (merge PR #26 або виділений PR). Запит власнику —
    `PANEL-leads-db.md` кінець файлу.
  - **Блок 5 — відновлення знімка з видаленими `_pub`.** Ізольований clone:
    відкат знімка `57c2a07` (посилається на видалений `_pub/8e9d45ba`): лише
    `published.json` → guard ✗; `+review-state.json` → ✗ (`файл відсутній`);
    `+ теку зображень` → **✓**, build OK, фото = байт-точна A. Точковий відкат
    = **3 об'єкти з одного SHA** (published + review-state + медіа знімка).
    Виправлено `PANEL-backup-restore.md` + `PANEL-etap4-protocol.md` §N.
  - **Блок 4 — мобільний Preview — ПЕРЕВАЖНО ЗАКРИТО живим власником.** Обидва
    браузерні інструменти без GitHub-сесії Preview цієї сесії; але власник сам
    надіслав 3 знімки з **реального iPhone** (Safari, авторизовано): Keystatic
    «Послуги → Додати», навігаційна шухляда, мовна секція форми — усе без
    горизонтального скролу, поля читабельні. Закриває основний ризик (§J).
    Дрібний залишок: сам `/panel`-дашборд (картки з кнопками) на реальному
    пристрої ще не сфотографовано — не блокер, локальна емуляція (§K) вже це
    покриває на тому самому коді.
    **Побічна перевірка безпеки:** власник натиснув «Створити» на незаповненій
    формі («проба» — власний тестовий текст) — Keystatic заблокував запис
    клієнтською валідацією; звірено `git rev-parse` на обох гілках (`main`
    `ce1977af…`, `codex/admin-panel-spike` `a7864be…`) — жодних нових комітів,
    жодних слідів «проба» окрім наявних збігів у прозі документації.
- **Рекомендований ОДИН наступний етап: Б3-preview** (Neon Free, лише Preview).
  Відео-Blob (Б2) — після, разом із рішенням про Vercel Pro (ліміт Data
  Transfer 10 ГБ/міс на Hobby, `PANEL-video-hosting.md`). Не разом.
...
## Ревізія П44 — задача 16:08 (2026-09-10)

Пакет незалежних робіт після звіту 16:00. Код-HEAD `5221b83` → `1f6b86f`.

- **Блок 2 — «демо / не налаштовано» в заявках.** Звірено `resolveLeadsMode()`
  + `/panel/leads`: код **правильний** — hosted (`NODE_ENV==="production"`) без
  `LEADS_DATABASE_URL` → `not-configured` (блок «Сховище заявок не
  налаштоване»), **не** демо. Демо лише в локальному dev або за явним
  `LEADS_DEMO_MODE=1` з банером. Дефект був у **тексті доків** — виправлено в
  `PANEL-leads-db.md` (3 місця казали «показує демо-дані» на hosted). Додано
  2 тести (`store.test.ts`).
- **Блок 3 — шлях заявки Production vs Preview.** Дві схеми + точні файли в
  `PANEL-leads-db.md`. **Ключова відповідь: НІ** — Neon, підключений лише до
  Preview, **не** показує заявки з поточного Production (`LEADS_DATABASE_URL`
  per-environment; Production `/api/contact` не має кроку БД). Розбито пакет:
  Б3-preview (цей етап) vs Б3-prod (окреме рішення власника, GDPR).
- **Блок 4 — готовність адаптера заявок.** Переглянуто `schema.sql`,
  `leads-migrate.mjs`, `postgres.ts`, ідемпотентність, обробку помилок.
  - **Дефект виправлено:** `/panel/leads` рендерив сиру помилку `pg`/DNS
    (хост/порт/роль/назва БД). Новий `src/lib/leads/loadError.ts` — фіксований
    текст у UI + коарс-код у лог. Тести `loadError.test.ts` (7).
  - **Права `leads_app`:** адаптер робить лише `SELECT`+`INSERT` (ніколи
    `UPDATE`). Прибрано зайвий `GRANT UPDATE(deleted_at)`; задокументовано
    точні запити рантайму. Soft-delete = ручна дія власника міграційною роллю.
  - Узгоджено з кодом: «коли заявку прийнято» (хоча б один тривкий канал),
    «що таке повтор» (10–20 хв вікно, email — щоразу), «email не губиться при
    збої БД» (`route.ts` ковтає помилку, email далі).
  - Рекомендація (НЕ зроблено): додати `statement_timeout` — рішення власника.
  - Тести postgres.test.ts +3 (db-throw, порожній список, помилка читання).
- **Блок 6 — перевірка процедури відновлення контенту.** Ізольований
  `git clone` у scratchpad (реальний репо/гілка/сервери не чіпалися).
  **Знахідка:** документована процедура «відкат усього знімка» —
  `git checkout <sha> -- published.json` **сама по собі валить `content:guard`**,
  якщо старий знімок містить згодом видалений матеріал (його review-хеші
  вичищені). **Виправлено доку** `PANEL-backup-restore.md`: відкат
  `published.json` **І** `review-state.json` з того самого SHA → guard ✓
  (звірено: `803361e` лише published → ✗ на `zzz-test-panel`; обидва → ✓,
  53/53 медіа). Розмежовано git-відкат коду vs бекап зовнішніх сховищ/секретів;
  додано коротку інструкцію власнику для захищеної копії секретів (агент її
  не створює).
- **Блок 7 — мобільний Preview.** Без повторних марних спроб. Playwright дає
  справжній CSS-в'юпорт (375/320 підтверджено на Preview-деплої:
  `/panel`, `/panel/leads`, `/keystatic` без-авторизації — **h-overflow немає**,
  `scrollWidth == clientWidth == innerWidth`; 1 console-помилка — CSP блокує
  `vercel.live/feedback.js`, не дефект). Але Playwright **не має GitHub-сесії** —
  **авторизований** дашборд `/panel` на Preview недосяжний. Блокер звужено до
  «авторизація на Preview», не «немає інструмента в'юпорту». Лишається pending;
  потрібен 1 знімок власника (Chrome на Mac, DevTools device toolbar 375 + 320).
  Знімки: `~/Desktop/DREAM.CAR.VAVD-panel-samples-20260910/task-1608/`.
- **Блок 1 — повний цикл заміни фото A→B у github-режимі на Preview (НОВЕ).**
  Синтетичний матеріал галереї `zzz-photo-ab`, дві однокольорові тестові фото
  без PII (A синя `8e9d45ba…`, B помаранчева `b2cd3e64…`). Через інтерфейс:
  створити з A → 3× перевірка мов → опублікувати (сайт 3 мови = A, `_pub/<shaA>`)
  → замінити A→B у робочій копії → **знімок тримає A, чернетка показує B** →
  опублікувати B (сайт 3 мови = B, `_pub/<shaB>`, всі URL 200) → зняти →
  видалити в Keystatic → завершити видалення (обидві `_pub` прибрано).
  `git diff 91d6f81..HEAD -- src/content/ public/` — **лише `publishedAt`**;
  `review-state.json` — нуль-нетто. Деталі — `PANEL-etap4-protocol.md` §I2.
  Закриває пункт evidence-таблиці «повний github-mode Keystatic A→B e2e».
- **Блок 5 — пакети рішень.** `PANEL-leads-db.md`: оцінка навантаження
  (5–15 заявок/день → 0.5 ГБ на 100+ років; < 1 CU-год/міс), регіон
  (`eu-west-2`/`eu-central-1`), питання зберігання/GDPR для власника.
  `PANEL-video-hosting.md`: офіційні ліміти Vercel Blob (Hobby: 1 ГБ сховище,
  **10 ГБ transfer/міс** — вузьке місце; перевищення → сховище призупиняється),
  таблиця «що працює без сервісу / що потребує Blob / URL-досяжність чернеток
  (так, не погоджено) / видалення / бекап / звʼязок».
  **Рекомендація: один наступний етап — Б3-preview** (див. «Поточний стан»).

## Ревізія П44 — задача 20:24 (2026-09-10) — Б3-preview підготовка + залишки

Об'єднане завдання: підключення тестової БД + залишкові перевірки. Тільки
документи (жодних змін коду). HEAD `3c25963` + docs-коміт зверху.

- **Блок 2 — Neon → Preview: НЕ підключено, підготовлено.** Кроки фізично
  потребують дій, які асистент не має права робити: створення/вхід у
  Neon-акаунт (перевірено — `console.neon.tech` показує вхід, акаунта нема),
  введення `LEADS_DATABASE_URL` у Vercel (секрет; MCP-доступ до проєкту `dream
  .car.vavd` — **403**). Зроблено: звірка Neon Free (офіційно, 2026-09-10 —
  100 CU-год/проєкт, 0.5 ГБ, 5 ГБ transfer, scale-to-zero 5 хв, дані не
  видаляються при ліміті); runbook на 12 кроків (`PANEL-leads-db.md` розділ 5);
  перевірка **без надсилання форми** — синтетичний `create`/`list` через
  `getWritableLeadsStore` напряму (розділ 6 B/C/D).
- **`statement_timeout` — рішення прийнято.** `ALTER ROLE leads_app SET
  statement_timeout='4000ms'` + `idle_in_transaction_session_timeout='10s'` —
  параметр рівня ролі, **без зміни коду адаптера**, Neon-сумісно (vanilla PG),
  лише тестова runtime-роль. Додано до SQL створення ролі.
- **Блок 3 — синтетичні перевірки заявок.** Не виконано (немає БД). Порядок
  перевірок задокументовано: спершу email-маршрут (не надсилати),
  запис/ідемпотентність через адаптер, помилки без витоку хоста/ролі,
  прибирання. **Повний цикл форми з email — не перевірений** (поза дозволом).
- **Блок 6 — Production-план.** `origin/main` **не містить** subsystem заявок
  (0 файлів `panel|keystatic|leads` з 164). `main` `/api/contact` = лише email.
  Для Б3-prod: (a) код у `main`, (b) окрема prod-БД, (c) env Production, (d)
  redeploy, (e) GDPR. Запит власнику — `PANEL-leads-db.md` кінець файлу.
- **Блок 5 — відновлення знімка з видаленими `_pub`.** Ізольований clone:
  відкат `57c2a07` → guard ✗ доки не додано `published.json` +
  `review-state.json` + **тека медіа знімка** (`_pub/8e9d45ba` видалено
  `92c73d3`). Три об'єкти з одного SHA. Guard ✓ + build OK + фото байт-точні.
  `PANEL-backup-restore.md` + `PANEL-etap4-protocol.md` §N; запис «медіа
  гарантовано присутнє» (16:08) — застарів.
