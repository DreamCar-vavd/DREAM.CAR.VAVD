...
- **Remote HEAD:** `1f6b86f` — задача 16:08 (leads: hosted → not-configured;
  фікс витоку деталей підключення в `/panel/leads`; звірка прав `leads_app`;
  шлях заявки Prod vs Preview; перевірка процедури відновлення контенту;
  пакети рішень Б3/Б2). Попередній код-HEAD — `5221b83` (15:26).
  **416 тестів pass**, tsc 0, eslint 0, build OK, content:guard OK.
  CI + Vercel на `888967a` — success; на `1f6b86f` — чекається.
...
- **Блок 7 — мобільний Preview.** Без повторних марних спроб. Playwright дає
  справжній CSS-в'юпорт (375/320 підтверджено), але **не має GitHub-сесії** —
  авторизований `/panel` на Preview недосяжний. Блокер звужено до «авторизація
  на Preview», не «немає інструмента в'юпорту». Лишається pending; потрібен
  1 знімок власника (Chrome на Mac, DevTools device toolbar 375 + 320).
