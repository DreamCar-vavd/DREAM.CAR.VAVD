...
- **Remote = Local HEAD:** `1622059` (П38 — життєвий цикл карток і фото:
  §3 прив'язка підтверджень до примірника, §4 «Завершити видалення»,
  §5–6 заморозка опублікованих фото; 2026-09-09 ~17:00 BST; **запушено**,
  CI `Verify` + Vercel Preview — success на `1214411` та попередніх).
  `1622059` = 6 тест-комітів живої перевірки поверх `1214411`; **вміст
  `1622059` == `1214411`** (`git diff` порожній). `main` =
  ...
  `docs/PANEL-service-photos.md` (фото + §5–6), `docs/PANEL-owner-guide.md`.
  **Стан реального контенту CMS = початковому** (звірено вмістом): `published.json`
  тексти/структура = `604c27e` окрім (а) `publishedAt` (штатно) та (б) шляхів
  фото авто/галереї → заморожені `…/<slug>/_pub/<hash>.<ext>` копії (байт-ідентичні
  оригіналам; П38 §5–6). `review-state.json` — рядки +`instance` (П38 §3),
  хеші не змінені. Робочі картки +`bornAt` (П38 §3). Тестові коміти в історії.
