...
- **Remote HEAD:** `<нова після push>` — задача 16:08. Останній **код** —
  `1f6b86f` (leads: hosted → not-configured; фікс витоку деталей підключення в
  `/panel/leads`; звірка прав `leads_app`). Далі: `91d6f81` (docs Б3/Б2/restore),
  потім 13 Keystatic-комітів циклу заміни фото A→B (`8b26ae1`…`92c73d3`,
  **нуль-нетто** крім `publishedAt`), потім docs 16:08 §1/§7.
  **416 тестів pass**, tsc 0, eslint 0, build OK, content:guard OK.
  CI + Vercel на `888967a`/`1f6b86f` — success; на фінальному SHA — чекається.
- **Рекомендований ОДИН наступний етап підключення (задача 16:08 блок 5):**
  **Б3-preview — БД заявок (Neon Free, тестова гілка, лише Preview).**
  Чому саме він: одна змінна середовища, код готовий і звірений, нуль дотику
  до `main`/Production/публікації, найменший ризик, одразу дає власнику
  видимий результат (заявки в панелі). Відео-Blob (Б2) — після, і, ймовірно,
  разом із рішенням про Vercel Pro через ліміт Data Transfer 10 ГБ/міс на
  Hobby (`PANEL-video-hosting.md`). Не підключати обидва одночасно.
