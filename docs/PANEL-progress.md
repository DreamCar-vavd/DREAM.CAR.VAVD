...
- **Remote HEAD:** `865eddd` (П41 — узгоджений знімок для очищення + постер відео;
  задача 2026-09-10 09:01) ← `62c8554` ← `5e46cc3` (док П40) ← `ee93d24` (П40 код)
  ← `8827b1b` ← `f80fab7` ← `115520c`. Після `865eddd` — 3 коміти живої
  перевірки очищення (`test(zzz): … _pub copy A/B` + `panel: drop 2 unreferenced
  media file(s)`): **чистий діф `865eddd..HEAD` порожній** — реальний контент і
  код не зрушені; синтетичні `zzz` файли додано й прибрано. Останній не-тестовий
  SHA — `865eddd`. П38 (`1214411`) — базовий пайплайн. `main` =
  ...
  CI Verify + Vercel Preview build `865eddd` — **success**. tsc 0 · eslint 0 ·
  352 тестів pass · `npm run build` — Compiled successfully.
...
**Відкриті питання після П41 (справжні незакриті):**
- **Холодний старт `/panel` ще ~3.7 с.** П41 виміряв на Preview: cold 3.7 с,
  warm ~2.0 с (новий код). Cold містить init функції Vercel + холодні з'єднання
  з GitHub — оптимізація мережі (29 звернень, 0.15 МБ) на це не впливає прямо.
  Далі можна зменшувати cold (менше послідовних `readDir`, кеш дерева в межах
  запиту) — окреме завдання.
- **`posterSrc` 2 реальних авто ще плоский.** П41 §4: механізм заморозки
  постера готовий і покритий; але `dacia-sandero-2022` / `-comfort-2019` мають
  `video.posterSrc` = `photos/0/image.jpg` у `published.json` (фото 0 вже
  заморожене). Заморозиться при наступній публікації того авто. **Міграції
  свідомо немає** — обсяг мізерний (2 поля), плоскі файли на місці, дифер лишає
  їх «in-sync». Якщо власник хоче закрити зараз — одна публікація кожного з
  2 авто через `/panel`.
