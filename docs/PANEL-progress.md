...
- **Remote HEAD:** `ee93d24` (П40 — атомарне очищення `_pub` + визначення змін
  фото через git-дерево; задача 2026-09-09 18:51). Попереднє: `8827b1b` (П39
  код + док) ← `f80fab7` (фікс читання >1 МБ) ← `115520c` (П39). Коміти живої
  перевірки `zzz-*` самоскасовуються (вміст гілки поза `published.json.publishedAt`
  == останній не-тестовий SHA). П38 (`1214411`) — базовий пайплайн. `main` =
  ...
  CI Verify + Vercel Preview build `ee93d24` — **success**. tsc 0 · eslint 0 ·
  340 тестів pass · `npm run build` — Compiled successfully.
...
**Відкриті питання після П40 (справжні незакриті):**
- **Час завантаження `/panel` наживо не міряний.** П40 виміряв звернення до
  GitHub і обсяг даних (детерміновано: 83→29 звернень, 39.7→0.15 МБ). Стінний
  час на автентифікованій Preview-панелі, cold vs warm — потрібна сесія
  власника; окремо не проганявся.
- **`posterSrc` відео не заморожується.** У `published.json` поле `posterSrc`
  (постер відео авто) вказує на плоский `…/photos/0/image.jpg`, не на `_pub`.
  Файл на місці, сайт працює; але якщо Keystatic перенумерує/видалить те фото,
  постер може 404-нути. `stripPhotoUrls` порівнює `posterSrc` точно (не через
  git-id), тож фантомів це не дає. Заморозити `posterSrc` — окреме дрібне
  завдання.
- **Повний цикл github-режимного `publish` на Preview** (реальний коміт із
  панелі + послідовність A→B між двома збірками Vercel) — досі не проганявся як
  єдиний наскрізний сценарій (дав би ~4 коміти + 2 збірки). Механізм задеплоєно,
  покрито тестами; `/panel` на Preview без фантомних «є зміни».
