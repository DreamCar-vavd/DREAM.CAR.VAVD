...
- **Remote HEAD:** `115520c` (П39 код) + `ffec…`/наступний (доопрацювання за
  Preview) + коміти живої перевірки `zzz-*` (самоскасовуються — вміст гілки поза
  `published.json.publishedAt` == останній не-тестовий SHA). П38 (`1214411`) —
  базовий пайплайн. `main` = `ce1977af140b49dce4bb79001c7eeed5e01aa2c2` — **не
  зрушив** за всю роботу. PR #26 draft. branch alias
  `dreamcarvavd-git-codex-admin-p-648563-6y7h9wdz4r-7375s-projects.vercel.app`.
  Фінальний не-тестовий SHA — у завершальному повідомленні сесії.
...
**Відкриті питання після П38 (справжні незакриті):**
- **Заморозка фото на Preview — лише локально.** Повний цикл `freezeItemMedia`
  на github-режимному `publish` + послідовність A→B між двома реальними збірками
  Vercel не проганявся (дав би ~4 коміти + 2 збірки). Локально на
  `LocalFsStorage` + реальних jpg — пройдено; механізм задеплоєно, `/panel` на
  Preview без фантомних «є зміни».
- **§3 залишковий нюанс:** якщо картку **до П38** підтвердили (рядок без
  `instance`), а тепер видалять+створять наново → legacy-рядок (`instance:""`)
  проти нового `bornAt` → `"" !== "токен"` → потребує перевірки ✅. Але після
  міграції `legacy-<slug>` таких рядків у продакшені немає.
- **§4:** `completeDeletion` не робить GC `_pub/` тек для повністю видалених
  карток — але їх `_pub` файли вже прибирає `unpublishItem` (або їх не було,
  якщо картку не публікували). Витоку немає.
...
- **Розмір репо:** +~31 МБ (заморожені копії 11 наявних items). Старі копії
  GC-ляться при зміні фото; історія git їх зберігає назавжди.
