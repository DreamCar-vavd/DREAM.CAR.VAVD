...
- **Remote HEAD:** `866b271` — П44 Е1–Е4 (частково), задача 2026-09-10 11:49.
  ...
  **РЕВІЗІЯ СТАТУСУ (задача 2026-09-10 12:50):** попередній хендоф казав «Е1–Е3
  закінчені» — це **завищено**. Фактично:
  - **Е3 потребує доопрацювання.** Серверний `WriteUncertainError` досі
    повертається як звичайний JSON `transient:true` — тим самим прапорцем, що й
    rate-limit / unavailable; окремої машинної ознаки «результат запису
    невідомий» немає. Клієнтська «Перевірити результат» робить висновок
    «застосовано/НЕ» лише з `stateToken` (`publishState` + `publishedExists` +
    к-сть стале-slug) — не з конкретного очікуваного результату дії.
  - **Е4 виконаний частково** (локальні A–D + Preview-цикл через POST). Бракує:
    симв-посилання в guard шляхів; прив'язка плану очищення до SHA гілки (а не
    `publishedVersion` — версії одного файла); знімки Preview; цикл через кнопку
    UI; цільові тести поведінки (не лише текстів).
  - **Е5 (інструкція власнику) — доповнити.**
  Уже отримані докази (локальні A–D, Preview-цикл `866b271`, заміри Е2) —
  **зберігаються**, не перепроганяються без причини.
  **НАСТУПНИЙ КРОК:** див. запис «Ревізія П44» нижче — етапи 2–7 задачі 12:50.
...
**Етапи (кожен — окремий коміт + звіт):**
1. ✅ Ревізія статусу в документах (цей запис) — коміт нижче.
2. Серверна модель помилок: `outcome: "written" | "not-written" | "unknown"`
   (машинна ознака) наскрізь `panelStore` → `/api/panel` → клієнт. `unknown`
   блокує повторну зміну незалежно від ділянки обриву (браузер→сервер чи
   сервер→GitHub). «Перевірити результат» — лише читання.
3. Достовірна перевірка результату: звіряти конкретний очікуваний ефект
   (матеріал+версія для publish; мова+хеш тексту для confirm; конкретні
   записи/файли для видалення; саме шляхи плану для очищення). Неможливо
   достовірно → «Результат поки невідомий», блок лишається; невдала перевірка
   не знімає блок; завершення refresh ≠ доказ.
4. Ізоляція тестів: guard від виходу через symlink (тест — лише в пісочниці);
   план очищення несе SHA знімка гілки (не `publishedVersion`); атомарне
   видалення + серверна перевірка застарілого SHA — збережені.
5. Цільові тести поведінки (не текстів): 8 сценаріїв із задачі + лічильник
   запитів (немає повторного запису до підтвердження результату).
6. Жива перевірка Preview через кнопки на `zzz-test-panel`; свіжі знімки
   (широкий + 375 px); штучні обриви — локально з підписом; звірка
   початкового/кінцевого вмісту.
7. Інструкція власнику + журнал + протокол Е4 + хендоф + опис PR #26; прибрати
   застарілі твердження про непрацюючий вхід.

**Межі:** без змін `main`/Production/DNS/реальних матеріалів/тарифів/доступів/
секретів; сервери `:3000`/`:3010` не чіпати; патч Dacia не застосовувати;
Blob/Neon/content-guard→main — поза завданням; без force-push і руйнівного reset.
