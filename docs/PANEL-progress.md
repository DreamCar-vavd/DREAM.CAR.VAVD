...
**Дата:** 2026-09-09. **Коміт:** на гілці `codex/admin-panel-spike`, локально, не запушено. **Гілка:**
`codex/admin-panel-spike` (PR #26 draft), `main` не чіпається.
...
**Тести:** `src/lib/content/panelStore.test.ts` +4 — стале-рядок не гейтить і
потрапляє в `staleReviewSlugs`; `confirmLocale` прибирає його тим самим записом;
наново створений slug стартує з «потребує перевірки»; при недоступних читаннях
підтвердження не втрачається. **280 тестів, tsc 0, eslint 0.**

**Межа:** прибирання відкладене — стається на **наступному** «Позначити
перевіреним» по будь-якому матеріалу (не миттєво після видалення). Якщо картку
