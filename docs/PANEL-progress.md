...
|---|---|---|
| Тепле (усталене, 3–5 навігацій) | ~2.1–3.0 с (сер. ~2.55 с) | **~1.4–1.6 с (сер. ~1.5 с)** |
| Холодне | 7.2 с (після ~1 год простою) | 2.4 с (свіжий деплой — міг бути прогрітий Vercel); окремий idle-cold — ↓ |
| Дані з GitHub / рендер | 0.15 МБ | 0.15 МБ (без змін — те саме дерево) |
| Звернень до GitHub / рендер | 29 | 29 (та сама кількість, але тепер до 8 паралельно, не поспіль) |

**Тепле відкриття: −~40 % (2.55 → 1.5 с)** за однакових умов — завдяки
паралельному читанню JSON-файлів у `readDir`. Кількість звернень і обсяг даних
не змінилися; змінилася лише їх серіалізація. Холодне idle-cold порівняння —
див. нижче (окремий вимір після простою).
